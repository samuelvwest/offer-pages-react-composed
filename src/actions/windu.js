export const initiateWindu = function() {
    if (!window.windu) {
        window.windu = {
            rC: function () {
                let char = '';
            ...
            sC: function (p) {
                let ol = 24;
                let init = Math.floor((1 + Math.random()) * 0x10000) * Date.now();
                let iS = init.toString();
                let iCN = Math.floor(12 + Math.random() * 42);
                if (p === 'long') {
                    ol = 6;
                ...
                for (let i = 0; i < iCN; i++) {
                    let iL = iS.length;
                    let iP = Math.floor(1 + Math.random() * iL);
                    iS = [iS.slice(0, iP), this.rC(), iS.slice(iP)].join('');
                }
                let sub = Math.floor(1 + Math.random() * (iS.length - ol));
                let use = iS.substring(sub, sub + ol);
                return use;
            },
            rKG: function (pref) {
                if (pref === 'long') {
